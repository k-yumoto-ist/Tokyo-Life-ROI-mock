...
function PlanPreview({ plan, selected }: { plan: SimplePlan; selected?: boolean }) {
  return <article className={`simple-experience-plan-card accent-${plan.accent} ${selected ? "is-selected" : ""}`}><div className="simple-experience-plan-card-head"><span>{plan.label}</span>{selected && <CheckCircle2 size={18} />}</div><h2>{plan.title}</h2><p className="simple-experience-place"><MapPin size={15} />{plan.place}</p><div className="simple-experience-plan-facts"><span><Clock3 size={15} />{plan.time}</span><span><WalletCards size={15} />{plan.cost}</span></div><p className="simple-experience-plan-summary">{plan.summary}</p></article>;
}
